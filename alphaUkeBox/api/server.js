  ...
  if (token) {
    ...
    User.findOne({
      ...
      if (!user) {
        ...
        return res.json({success: true, msg: 'Congrats on logging in successfully, ' + user.name + '!'});
      }
    });
  } else {
...
// Start the server
app.listen(port);
console.log('Go get yourself a cup of tea and check out: http://localhost:' + port);
